...
static void displayThread_func(ULONG thread_input);
static uint8_t displayThread_stack[1024] BSP_PLACE_IN_SECTION_V2(".stack.displayThread") BSP_ALIGN_VARIABLE_V2(BSP_STACK_ALIGNMENT);
...
TX_QUEUE gSetPointDisplayQueue;
static uint8_t queue_memory_gSetPointDisplayQueue[20];
TX_QUEUE gSpeedDisplayQueue;
static uint8_t queue_memory_gSpeedDisplayQueue[20];
TX_QUEUE gDutyCycleDisplayQueue;
static uint8_t queue_memory_gDutyCycleDisplayQueue[20];
...
void displayThread_create(void)
{
    ...
    UINT err_gSetPointDisplayQueue;
    err_gSetPointDisplayQueue = tx_queue_create (&gSetPointDisplayQueue, (CHAR *) "SetPointDisplayQueue", 1,
                                                 &queue_memory_gSetPointDisplayQueue,
                                                 sizeof(queue_memory_gSetPointDisplayQueue));
    if (TX_SUCCESS != err_gSetPointDisplayQueue)
    {
        tx_startup_err_callback (&gSetPointDisplayQueue, 0);
    }
    UINT err_gSpeedDisplayQueue;
    err_gSpeedDisplayQueue = tx_queue_create (&gSpeedDisplayQueue, (CHAR *) "SpeedDisplayQueue", 1,
                                              &queue_memory_gSpeedDisplayQueue,
                                              sizeof(queue_memory_gSpeedDisplayQueue));
    if (TX_SUCCESS != err_gSpeedDisplayQueue)
    {
        tx_startup_err_callback (&gSpeedDisplayQueue, 0);
    }
    UINT err_gDutyCycleDisplayQueue;
    err_gDutyCycleDisplayQueue = tx_queue_create (&gDutyCycleDisplayQueue, (CHAR *) "DutyCycleDisplayQueue", 1,
                                                  &queue_memory_gDutyCycleDisplayQueue,
                                                  sizeof(queue_memory_gDutyCycleDisplayQueue));
    if (TX_SUCCESS != err_gDutyCycleDisplayQueue)
    {
        tx_startup_err_callback (&gDutyCycleDisplayQueue, 0);
    }

    UINT err;
    err = tx_thread_create (&displayThread, (CHAR *) "DisplayThread", displayThread_func, (ULONG) NULL,
                            &displayThread_stack, 1024, 3, 3, 10, TX_AUTO_START);
    if (TX_SUCCESS != err)
    {
