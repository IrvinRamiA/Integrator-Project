...
#endif
TX_QUEUE displayThread_message_queue;
static uint8_t queue_memory_displayThread_message_queue[SF_MESSAGE_CFG_QUEUE_SIZE];
static sf_message_subscriber_t displayThread_message_queue_0_0 =
{ .p_queue = &displayThread_message_queue, .instance_range =
{ .start = 0, .end = 0 } };
static sf_message_subscriber_t *gp_group_SF_MESSAGE_EVENT_CLASS_TOUCH[] =
{ &displayThread_message_queue_0_0, };
...
void g_message_init(void)
{
    tx_queue_create (&displayThread_message_queue, (CHAR *) "DisplayThread Message Queue", 1,
                     &queue_memory_displayThread_message_queue, sizeof(queue_memory_displayThread_message_queue));
}
